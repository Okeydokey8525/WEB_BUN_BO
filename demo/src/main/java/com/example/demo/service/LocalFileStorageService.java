...
public class LocalFileStorageService implements FileStorageService {
    ...
    public String storeImage(MultipartFile file, String category) {
        ...
        String original=Optional.ofNullable(file.getOriginalFilename()).orElse(""); String ext=original.contains(".")?original.substring(original.lastIndexOf('.')+1).toLowerCase(Locale.ROOT):""; if(!Set.of("jpg","jpeg","png","webp").contains(ext)) throw new FileStorageException("Định dạng ảnh không hợp lệ."); if(!validContentType(file.getContentType(), ext)) throw new FileStorageException("Kiểu tệp ảnh không hợp lệ.");
    ...
    private boolean validContentType(String contentType, String extension) { if (contentType == null) return false; return switch (extension) { case "jpg", "jpeg" -> "image/jpeg".equalsIgnoreCase(contentType); case "png" -> "image/png".equalsIgnoreCase(contentType); case "webp" -> "image/webp".equalsIgnoreCase(contentType); default -> false; }; }
}
